...
#ifdef __LP64__
    //64-bit Intel or PPC
    output->id = (uint64) oldestFrame->base_dpb; /* use the pointer as the id */
#else
...
OSCL_EXPORT_REF void PVAVCCleanUpDecoder(AVCHandle *avcHandle)
{
    ...
    if (decvid != NULL)
    {
...
#ifdef __LP64__
            //64-bit Intel or PPC
            avcHandle->CBAVC_Free(userData, (int64)video); /* last thing to do */
#else
            //32-bit Intel, PPC or ARM
...
#ifdef __LP64__
                    //64-bit Intel or PPC
                    avcHandle->CBAVC_Free(userData, (int64)decvid->picParams[i]->slice_group_id);
#else
                    //32-bit Intel, PPC or ARM
...
#ifdef __LP64__
                //64-bit Intel or PPC
                avcHandle->CBAVC_Free(userData, (int64)decvid->picParams[i]);
#else
                //32-bit Intel, PPC or ARM
        ...
        for (i = 0; i < 32; i++)
        {
            ...
            {
#ifdef __LP64__
                avcHandle->CBAVC_Free(userData, (int64)decvid->seqParams[i]);
                //64-bit Intel or PPC
#else
                //32-bit Intel, PPC or ARM
...
#ifdef __LP64__
            //64-bit Intel or PPC
            avcHandle->CBAVC_Free(userData, (int64)decvid->bitstream);
#else
            //32-bit Intel, PPC or ARM
...
#ifdef __LP64__
        //64-bit Intel or PPC
        avcHandle->CBAVC_Free(userData, (int64)decvid);
#else
