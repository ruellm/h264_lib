...
#ifdef __LP64__
            //64-bit Intel or PPC
            avcHandle->CBAVC_Free(userData, (uint64)video->mblock);
#else
...
#ifdef __LP64__
            //64-bit Intel or PPC
            avcHandle->CBAVC_Free(userData, (uint64)video->MbToSliceGroupMap);
#else
            //32-bit Intel, PPC or ARM
...
#ifdef __LP64__
            //64-bit Intel or PPC
            avcHandle->CBAVC_Free(userData, (int64)dpb->fs[ii]);
#else
            //32-bit Intel, PPC or ARM
