    ...
    switch (encvid->enc_state)
    {
        ...
        case AVCEnc_Encoding_SPS:
            ...
            if (status == AVCENC_SUCCESS)
            {
				...
				video->currPicParams->pic_parameter_set_id = 0;
...
#ifdef __LP64__
    //64-bit Intel or PPC
    recon->id = (uint64) currFS->base_dpb; /* use the pointer as the id */
#else
...
#ifdef __LP64__
            //64-bit Intel or PPC
            avcHandle->CBAVC_Free(userData, (int64)encvid->functionPointer);
#else
            //32-bit Intel, PPC or ARM
...
#ifdef __LP64__
            //64-bit Intel or PPC
            avcHandle->CBAVC_Free(userData, (int64)encvid->min_cost);
#else
            //32-bit Intel, PPC or ARM
...
#ifdef __LP64__
            //64-bit Intel or PPC
            avcHandle->CBAVC_Free(userData, (int64)encvid->intraSearch);
#else
            //32-bit Intel, PPC or ARM
...
#ifdef __LP64__
            //64-bit Intel or PPC
            avcHandle->CBAVC_Free(userData, (int64)encvid->mot16x16);
#else
            //32-bit Intel, PPC or ARM
...
#ifdef __LP64__
            //64-bit Intel or PPC
            avcHandle->CBAVC_Free(userData, (int64)encvid->rateCtrl);
#else
            //32-bit Intel, PPC or ARM
...
#ifdef __LP64__
            //64-bit Intel or PPC
            avcHandle->CBAVC_Free(userData, (int64)encvid->overrunBuffer);
#else
            //32-bit Intel, PPC or ARM
...
#ifdef __LP64__
                //64-bit Intel or PPC
                avcHandle->CBAVC_Free(userData, (int64)video->MbToSliceGroupMap);
#else
                //32-bit Intel, PPC or ARM
...
#ifdef __LP64__
                //64-bit Intel or PPC
                avcHandle->CBAVC_Free(userData, (int64)video->mblock);
#else
                //32-bit Intel, PPC or ARM
...
#ifdef __LP64__
                //64-bit Intel or PPC
                avcHandle->CBAVC_Free(userData, (int64)video->decPicBuf);
#else
                //32-bit Intel, PPC or ARM
...
#ifdef __LP64__
                //64-bit Intel or PPC
                avcHandle->CBAVC_Free(userData, (int64)video->sliceHdr);
#else
                //32-bit Intel, PPC or ARM
...
#ifdef __LP64__
                    //64-bit Intel or PPC
                    avcHandle->CBAVC_Free(userData, (int64)video->currPicParams->slice_group_id);
#else
                    //32-bit Intel, PPC or ARM
...
#ifdef __LP64__
                //64-bit Intel or PPC
                avcHandle->CBAVC_Free(userData, (int64)video->currPicParams);
#else
                //32-bit Intel, PPC or ARM
...
#ifdef __LP64__
                //64-bit Intel or PPC
                avcHandle->CBAVC_Free(userData, (int64)video->currSeqParams);
#else
                //32-bit Intel, PPC or ARM
...
#ifdef __LP64__
                //64-bit Intel or PPC
                avcHandle->CBAVC_Free(userData, (int64)encvid->bitstream);
#else
                //32-bit Intel, PPC or ARM
...
#ifdef __LP64__
                //64-bit Intel or PPC
                avcHandle->CBAVC_Free(userData, (int64)video);
#else
                //32-bit Intel, PPC or ARM
...
#ifdef __LP64__
        //64-bit Intel or PPC
        avcHandle->CBAVC_Free(userData, (int64)encvid);
#else
        //32-bit Intel, PPC or ARM
