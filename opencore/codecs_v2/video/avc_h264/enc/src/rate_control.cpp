...
#ifdef __LP64__
        //64-bit Intel or PPC
        avcHandle->CBAVC_Free(avcHandle->userData, (int64)(rateCtrl->MADofMB));
#else
...
#ifdef __LP64__
                    //64-bit Intel or PPC
                    avcHandle->CBAVC_Free(avcHandle->userData, (int64)rateCtrl->pMP->pRDSamples[i]);
#else
                    //32-bit Intel, PPC or ARM
...
#ifdef __LP64__
            //64-bit Intel or PPC
            avcHandle->CBAVC_Free(avcHandle->userData, (int64)rateCtrl->pMP->pRDSamples);
#else
            //32-bit Intel, PPC or ARM
...
#ifdef __LP64__
        //64-bit Intel or PPC
        avcHandle->CBAVC_Free(avcHandle->userData, (int64)(rateCtrl->pMP));
#else
        //32-bit Intel, PPC or ARM
