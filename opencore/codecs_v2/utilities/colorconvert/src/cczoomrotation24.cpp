...
int32 ColorConvert24::GetOutputBufferSize(void)
{
    ...
    if (_mIsZoom)
    {
        return  _mState ? ((_mDst_height + 1)*_mDst_pitch*3) : (_mDst_pitch*_mSrc_height*3);
    }
    else
    {
        return  _mState ? ((_mDst_height)*_mDst_pitch*3) : (_mDst_pitch*_mSrc_height*3);
    }
}
...
/*---------------------------------------------------------------------------------
 * CC24 WITH ARMV6 ASSEMBLY, FULL RANGE
 *---------------------------------------------------------------------------------
 */
...
#ifdef __LP64__
                //64-bit Intel or PPC
                if (((uint64)pDst)&1) /* not word-aligned */
#else
                //32-bit Intel, PPC or ARM
