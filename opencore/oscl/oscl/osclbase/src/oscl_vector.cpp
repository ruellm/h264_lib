...
#ifdef __LP64__
    //64-bit Intel or PPC
    numelems -= (((uint64)last - (uint64)first) / sizeof_T);
#else
...
#ifdef __LP64__
        //64-bit Intel or PPC
        OsclAny* tmp = pOpaqueType->allocate((uint64)x.end() - (uint64)x.begin());
#else
        //32-bit Intel, PPC or ARM
...
#ifdef __LP64__
    //64-bit Intel or PPC
    oscl_memmove(result, first, (uint64)last - (uint64)first);
#else
    //32-bit Intel, PPC or ARM
